...
void vector_test() {
    ...
    imc_ft_vector_t* vectorArray[MAX_ADD+1];
    imc_ft_vector_t* vectorArray2[MAX_ADD+1];;
    
    // create & push
    vectorArray[0] = imc_ft_vector_create();
    vectorArray2[0] = imc_ft_vector_create();
    for (int i = 1; i < MAX_ADD+1; i++) {
        vectorArray[i] = imc_ft_vector_push(vectorArray[i-1], ((void*)&data[i-1]));
        vectorArray2[i] = imc_ft_vector_push(vectorArray2[i-1], ((void*)&data2[i-1]));
    }
    imc_ft_vector_t* max_vector = vectorArray[MAX_ADD];
    imc_ft_vector_t* max_vector2 = vectorArray2[MAX_ADD];
    
    // size
    assert(imc_ft_vector_size(max_vector) == MAX_ADD);
    
    // update
    imc_ft_vector_t* v_updated = imc_ft_vector_update(max_vector, MAX_ADD/2, &data_u);
    printf("\n===updated===\n");
    imc_ft_vector_dump(v_updated);
    
    // lookup
    assert(*imc_ft_vector_lookup(v_updated, MAX_ADD/2) == data_u);
    
    // pop
    imc_ft_vector_t* v_popped = imc_ft_vector_pop(max_vector, &data_p);
    assert(imc_ft_vector_size(v_popped) == MAX_ADD-1);
    assert(*data_p == (MAX_ADD-1)*5);
    
    // split
    imc_ft_vector_t *vec_out1 = NULL, *vec_out2 = NULL;
    imc_ft_vector_split(max_vector, MAX_ADD/2, &vec_out1, &vec_out2);
    
    // dump
    printf("===vec_out1===\n");
    imc_ft_vector_dump(vec_out1);
    printf("\n===vec_out2===\n");
    imc_ft_vector_dump(vec_out2);
    
    // merge
    imc_ft_vector_t *merged = imc_ft_vector_merge(max_vector, max_vector2);
    printf("\n===max_vector===\n");
    imc_ft_vector_dump(max_vector);
    printf("\n===max_vector2===\n");
    imc_ft_vector_dump(max_vector2);
    printf("\n===merged===\n");
    imc_ft_vector_dump(merged);
    printf("\n\n");
    
    // unref
    for (int i = 0; i < MAX_ADD+1; i++) {
        imc_ft_vector_unref(vectorArray[i]);
        imc_ft_vector_unref(vectorArray2[i]);
    }
    imc_ft_vector_unref(v_updated);
    imc_ft_vector_unref(v_popped);
    imc_ft_vector_unref(vec_out1);
    imc_ft_vector_unref(vec_out2);
    imc_ft_vector_unref(merged);
}
