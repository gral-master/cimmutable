...
double eval_vector_cmds(Prog* prog, command** cmds,
  ...
  for (int i = 0; i < size; i++) {
    ...
    switch (cmd->type) {
    case CREATE:
        vec[obj_out] = imc_rrb_create();
        break;
    
    case UNREF:
        imc_rrb_unref(vec[obj_in]);
        break;

    case UPDATE:
      vec[obj_out] = imc_rrb_update(vec[obj_in], cmd->index, &cmd->data.as_int);
      break;

    case PUSH:
      vec[obj_out] = imc_rrb_push(vec[obj_in], &cmd->data.as_int);
      break;

    case POP:
      ;void* data;
      vec[obj_out] = imc_rrb_pop(vec[obj_in], &data);
      break;

    case MERGE:
        vec[obj_out] = imc_rrb_merge(vec[obj_in], vec[obj_aux]);
        break;

    case SPLIT:
        imc_rrb_split(vec[obj_in], cmd->index, &vec[obj_out], &vec[obj_aux]);
        break;
        
    case LOOKUP:
        imc_rrb_lookup(vec[obj_in], cmd->index);
        break;

    case SIZE:
        imc_rrb_size(vec[obj_in]); // hopefully won't be optimized out.
        break;
    
    case DUMP:
        // Note: only dump is test mode.
        if (is_test)
            imc_rrb_dump(vec[obj_in]);
        break;
...
double execute_vector (Prog* prog) {
  imc_rrb_t** vec = malloc(prog->nb_var * sizeof(*vec));
